...
export interface Channel {
  name: string;
  urls: { url: string; label: string, hd: boolean, language: string }[];
  logo: string;
}
...
const originalChannels: { name: string; url: string; logo: string }[] = [
    ...
    { name: 'ESPN ARGENTINA', url: 'https://embed.ksdjugfsddeports.fun/embed/espnar.html', logo: 'https://a.espncdn.com/combiner/i?img=%2Fi%2Fespn%2Fespn_logos%2Fespn_red.png' },
    ...
    { name: 'National Geographic', url: 'https://embed.ksdjugfsddeports.fun/embed/natgeo.html', logo: 'https://upload.wikimedia.org/wikipedia/commons/2/22/Nat_Geo_HD.png' },
...
originalChannels.forEach(channel => {
    ...
    if (existing) {
        ...
        if (!existing.urls.some(u => u.url === channel.url)) {
            existing.urls.push({ url: channel.url, label: `Opción ${existing.urls.length + 1}`, hd: false, language: '' });
        }
    } else {
        mergedChannels.set(channel.name, {
            name: channel.name,
            logo: channel.logo,
            urls: [{ url: channel.url, label: 'Opción 1', hd: false, language: '' }],
        });
    }
});
