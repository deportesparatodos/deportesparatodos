...
export default function HomePage() {
  ...
  const { liveEvents, upcomingEvents, unknownEvents, finishedEvents, channels247, filteredChannels, searchResults, allSortedEvents, categoryFilteredEvents } = useMemo(() => {
    ...
    if (searchTerm) {
        ...
        const eventsSource = currentCategory 
            ? combinedEvents.filter(e => e.category.toLowerCase() === currentCategory.toLowerCase())
            : combinedEvents;

        const filteredEvents = eventsSource.filter(e => e.title.toLowerCase().includes(lowercasedFilter));
        const sChannels = currentCategory ? [] : channels.filter(c => c.name.toLowerCase().includes(lowercasedFilter));
    ...
    if (currentCategory) {
        categoryFilteredEvents = combinedEvents
            .filter(event => event.category.toLowerCase() === currentCategory.toLowerCase())
            .sort((a, b) => {
  ...
  }, [events, ppvEvents, searchTerm, currentCategory]);
  ...
  const pageTitle = currentCategory ? (
    <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold capitalize">{currentCategory}</h1>
    </div>
  ) : (
            ...
            <div className="space-y-2">
                {searchTerm ? (
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-4 md:gap-6 pt-4">
                     ...
                     <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-4 md:gap-6 pt-4">
                        {categoryFilteredEvents.map((event, index) => (
                           <EventCard
                                key={`cat-event-${index}`}
                                event={event}
                            ...
                            >
                                ...
                                <CarouselContent className="-ml-4">
                                    <CarouselItem className="basis-auto pl-4">
                                        <Button variant="secondary" className="h-12 px-6 text-lg" onClick={() => handleCategoryClick('en vivo')}>
                                            En Vivo
                                        </Button>
                                    </CarouselItem>
                                     <CarouselItem className="basis-auto pl-4">
                                ...
                                {categories.map((category) => (
                                    <CarouselItem key={category} className="basis-auto pl-4">
                                        <Button variant="secondary" className="h-12 px-6 text-lg" onClick={() => handleCategoryClick(category)}>
                                            {category}
                                        </Button>
                                    </CarouselItem>
                                ))}
